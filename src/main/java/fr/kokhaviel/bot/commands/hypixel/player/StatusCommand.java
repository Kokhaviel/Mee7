...
public class StatusCommand extends ListenerAdapter {
	...
	public void onMessageReceived(@NotNull MessageReceivedEvent event) {
		...
		if(args[0].equalsIgnoreCase(prefix + "status")) {
			...
			try {
				...
				channel.sendMessageEmbeds(getStatus(player, GENERAL_OBJECT).build()).queue();
	...
	public EmbedBuilder getStatus(PlayerData.Player player, JsonObject generalObject) throws MalformedURLException {
		EmbedBuilder hypixelEmbed = new EmbedBuilder();

		Session session = HYPIXEL_API.getStatus(player.getDisplayName()).getSession();

		hypixelEmbed.setAuthor("Hypixel Player Status", null, Config.HYPIXEL_ICON);
		hypixelEmbed.setColor(Color.CYAN);
		...
		hypixelEmbed.addField("Status : ", session.isOnline() ? "Online" : "Offline", false);

		if(session.isOnline()) {
			hypixelEmbed.addField("Game : ", session.getType(), true);
			hypixelEmbed.addField("Mode : ", session.getMode(), true);
			hypixelEmbed.addField("Map : ", session.getMap(), true);
		}
