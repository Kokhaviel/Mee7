...
public class RepeatCommand extends ListenerAdapter {
	...
	public void onMessageReceived(@NotNull MessageReceivedEvent event) {
		...
		if(args[0].equalsIgnoreCase(prefix + "repeat")) {
			...
			channel.sendMessageFormat("%s **%s**", MUSIC_OBJECT.get("player_set").getAsString(),
					newRepeat ? MUSIC_OBJECT.get("repeating").getAsString() : MUSIC_OBJECT.get("not_repeating").getAsString()).queue(
					delete -> delete.delete().queueAfter(5, TimeUnit.SECONDS));
		}
	}
}
