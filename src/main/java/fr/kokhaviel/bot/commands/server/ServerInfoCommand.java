    ...
    public void onMessageReceived(@NotNull MessageReceivedEvent event) {
        ...
        if(args[0].equalsIgnoreCase(Config.PREFIX + "serverinfo")) {
            ...
            if(args.length > 1) {
                ...
                event.getChannel().sendMessage("Too Arguments : Please Use " + Config.PREFIX + "serverinfo").queue(
                ...
                serverinfoEmbed.setTitle(guild.getName() + " Server Info")
                        ...
                        .setAuthor("Server Info", null, event.getJDA().getSelfUser().getAvatarUrl());
                ...
                if(guild.getDescription() != null) serverinfoEmbed.addField("Description : ", guild.getDescription(), false);
                serverinfoEmbed.addField("Boost Tier : ", guild.getBoostTier().name(), false);
                if(guild.getSystemChannel() != null) serverinfoEmbed.addField("System Channel : ", guild.getSystemChannel().getAsMention(), false);

                event.getMessage().delete().queueAfter(2, TimeUnit.SECONDS);

                event.getChannel().sendMessage(serverinfoEmbed.build()).queue();
